...
const MenuSquare = styled.View`
    ...
    height: 35px;
    margin-right: 20px;
    background-color: ${props=>props.active ? '#0098DA' : 'transparent'};
...
const MenuButtonText = styled.Text`
    font-size: 15px;
    margin-left: 10px;
    color: ${props=>props.active ? '#0098DA' : '#666E78'};
`;
                ...
                {menus.map((item, index)=>(
                    <MenuButton key={index} onPress={()=>navigation.navigate(item.screen)}>
                        ...
                        <Icon
                            name={item.icon}
                            size={20}
                            color={props.state.routes[props.state.index].name === item.screen ? '#0098DA' : '#666E78'}
                        />
                        <MenuButtonText
                            active={props.state.routes[props.state.index].name === item.screen}
